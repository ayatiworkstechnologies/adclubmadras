...
import { submitMembershipApplication , getOptions } from "@/api/api";
...
export default function MembershipForm() {
  ...
  const calculateTotalAmount = () => {
    const baseAmount = MEMBERSHIP_FEES[category];
    const extraNominees =
...
//       companyname_i: data.organization,
//       contactperson_i: data.contactPerson,
//       address1_i: data.address,
//       phoneno_i: data.telephone || "",
//       mobileno_i: data.mobile,
//       emailid_i: data.email,
//       fb_i: data.facebook || "",
//       twi_i: data.twitter || "",
//       ln_i: data.linkedin || "",
...
//       payload.nominees = data.nominees.map((nom) => ({
//         name: nom.name,
//         email: nom.email,
//         mobile: nom.mobile,
//       }));
...
//       const {
//         payment,
//         redirect_url, // this will be PayU's payment URL
//       } = res;
...
//       form.method = "POST";
//       form.action = redirect_url; // ✅ Use redirect_url from Laravel
...
const onSubmit = async (data) => {
  try {
    ...
    console.log("✅ Payload to Submit Membership API:", payload);

    const res = await submitMembershipApplication(payload);
    console.log("✅ Response from submitMembershipApplication:", res);
// debugger
    if (res?.payment && res?.redirect_url) {
      const {
        payment,
        redirect_url, // ✅ PayU Payment URL (Laravel returns it)
      } = res;
      ...
      console.log("✅ Final Fields for PayU Form Submission:", fields);
      ...
      document.body.appendChild(form);
      console.log("✅ Form Appended to Document Body, Submitting Now...");
      form.submit();
    } else {
      console.error("❌ Payment initiation failed. Response:", res);
      throw new Error("Payment initiation failed.");
    }
  } catch (error) {
  ...
  return (
    <section className="bg-black text-white mt-10 px-6 py-16">
      ...
      >
        <h2 className="text-3xl md:text-4xl font-bold  font-asgard text-primary mb-10 text-center uppercase">
          Membership Application Form
        </h2>

        <form
          onSubmit={handleSubmit(onSubmit)}
          className="space-y-10 font-glancyr"
        >
          {/* Category Selection */}
          <div>
